...
function register_my_custom_submenu_page() {
	...
	add_submenu_page( 'woocommerce', 'Reports-Prior', 'Reports-prior', 'manage_options', 'report-prior', 'my_custom_prior_submenu_page_callback' );
}
  ...
  echo '<input type="date" placeholder="From Date" id="dfatepicker" name="fdate" class="datepicker"  value="'.$_GET['fdate'].'" />';
   echo '<input type="date" id="tdatepicker" placeholder="To Date" name="tdate" class="datepicker" value="'.$_GET['tdate'].'" />';
	...
	//number,date,price and link
			...
			echo '<td>';
			echo date('m-d-Y',$oid['shipdate']).'</td>';
			echo '<td> <a target="_blank" href="'.get_edit_post_link($oid['order']->id).'">Edit Order</a>';
			echo '<td align="right">';
			echo $oid['order']->get_formatted_order_total().'</td>';
			echo '</td>';
			echo '</tr>';
			$csv_data[]=array($oid['order']->get_order_number(),date('m-d-Y',$oid['shipdate']),$oid['order']->get_total());
...
function my_custom_prior_submenu_page_callback() {
	...
	echo'<select style="text-transform:uppercase;vertical-align:top;" name="provider">';
	echo '	<option value="">All</option>';
	foreach($prior_report_provider as $key=>$value){
		echo '	<option ';
		if( $_GET["provider"] == $value )
		{
			echo 'selected="selected"';
	...
	foreach($prior_report_provider as $key=>$value){
		...
		$metaQuery = array();
		if(isset($_GET['provider']) && $_GET['provider']!='' && in_array($_GET['provider'],$prior_report_provider)) {
			$value = $_GET['provider'];
			$key = array_search($value, $prior_report_provider);
		}
